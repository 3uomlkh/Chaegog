    ...
    void uploader() {
        ...
        switch (imageFrom) {
            case GALLERY: // 갤러리에서 가져온 경우
                ...
                storageReference = firebaseStorage.getReference().child("Posts").child(firebaseUser.getUid()).child("Gallery").child(imageFileName); // reference에 경로 세팅
                uploadTask = storageReference.putFile(imageUri); // 업로드할 파일과 위치 설정
                break;

            case CAMERA: // 카메라에서 가져온 경우 createImage에서 생성한 이미지 우일명 이용
                storageReference = firebaseStorage.getReference().child("Posts").child(firebaseUser.getUid()).child("Camera").child(imageFile.getName()); // reference에 경로 세팅
