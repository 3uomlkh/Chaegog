    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < veganReasons.length; i++) {
            if (veganReasons[i].equals("환경")) {
                ...
                Log.d(TAG, "비건계기 배열 변경 오류");
            }
        }
        Log.d(TAG, Array_userVeganReason + " 입니다");
...
//                                                @Override
//                                                public void onComplete(@NonNull Task<Void> task) {
//                                                    Toast.makeText(RegisterStep4Activity.this, "인증 메일 발송", Toast.LENGTH_SHORT).show();
//                                                }
//                                            });
//
////                            Log.d(TAG, "회원가입 최종 성공");
////
////                            getFCMToken();
////
////                            // 처리중 화면 종료
        ...
        FirebaseMessaging.getInstance().getToken()
                .addOnCompleteListener(new OnCompleteListener<String>() {
                    ...
                    public void onComplete(@NonNull Task<String> task) {
                        if (!task.isSuccessful()) {
                            Log.d("FirebaseMessaging", "Fetching FCM registration token failed", task.getException());
                            return;
                        }
                        ...
                        Log.d(TAG, "비건계기 " + Array_userVeganReason + " 알러지 " + Array_userAllergy + "등록 성공");
                    }
                });
    }
